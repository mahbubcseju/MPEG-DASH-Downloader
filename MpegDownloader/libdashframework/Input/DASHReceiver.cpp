...
DASHReceiver::DASHReceiver          (IMPD *mpd, IDASHReceiverObserver *obs, MediaObjectBuffer *buffer, uint32_t bufferSize) :
              ...
              isBuffering           (false)
{
    std::cout<<"DASHReceiver :: COnstrutor "<<mpd<<std::endl;
   ...
   std::cout<<"lol koyta   aaaaaaaaaaaaaaaa"<<this->representationStream->GetSize()<<std::endl;
...
{
    ISegment *seg = NULL;
    std::cout<<"Get Next Segmnet 1"<<std::endl;
    std::cout<<this->segmentNumber<<" "<<representationStream->GetSize()<<std::endl;
    if(this->segmentNumber >= this->representationStream->GetSize())
        return NULL;

    std::cout<<"Get Next Segmnet"<<std::endl;
...
void                        DASHReceiver::DownloadInitSegment    (IRepresentation* rep)
{
    ...
    if (initSeg)
    {
        std::cout<<"Dash Receiever Download Init Segment "<<std::endl;
...
void*                       DASHReceiver::DoBuffering               (void *receiver)
{
    ...
    {
        media->StartDownload();
        std::cout<<"DASHReceiver::DoBuffering 3"<<std::endl;
        if (!dashReceiver->buffer->PushBack(media))
            return NULL;
        std::cout<<"DASHReceiver::DoBuffering 4"<<std::endl;
        media->WaitFinished();

        dashReceiver->NotifySegmentDownloaded();
          std::cout<<"DASHReceiver::DoBuffering 5"<<std::endl;
        media = dashReceiver->GetNextSegment();
        std::cout<<"Next media  "<<std::endl; 
           std::cout<<"DASHReceiver::DoBuffering 6"<<std::endl;
    }
      std::cout<<"DASHReceiver::DoBuffering 10"<<std::endl;
